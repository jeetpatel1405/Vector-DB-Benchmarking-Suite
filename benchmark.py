...
try:
    ...
    print("Warning: mplcyberpunk not available, using default matplotlib style")
...
def load_embeddings(parquet_path: str):
    ...
    for embedding_str in df["embedding"]:
        if isinstance(embedding_str, str):
            ...
            # Convert bytes to string and parse as list
            embedding_str = embedding_str.decode('utf-8')
            embedding_list = ast.literal_eval(embedding_str)
            embeddings_list.append(embedding_list)
        else:
            embeddings_list.append(embedding_str)
...
def main():
    ...
    ap.add_argument(
        "--force_recreate",
        action="store_true",
        help="Force recreate databases if dimension mismatch is detected. Default: False",
    )
    args = ap.parse_args()
